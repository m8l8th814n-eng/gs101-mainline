...
#include <linux/io.h>
/*
 * Mainline port: the vendor driver reached the PMU via the out-of-tree
 * soc/google/exynos-pmu-if.h (exynos_pmu_update/read). Mainline gs101 exposes
 * the PMU as a syscon regmap (compatible "google,gs101-pmu"), so the PHY
 * isolation is driven through samsung,pmu-syscon instead.
 */
		...
		struct mipi_phy_desc *phy_desc, unsigned int on)
{
	...
	ret = __set_phy_isolation(state->reg_pmu, phy_desc->iso_offset, on);
...
static int exynos_mipi_phy_probe(struct platform_device *pdev)
{
	...
	/* PMU syscon for PHY isolation (google,gs101-pmu) */
	state->reg_pmu = syscon_regmap_lookup_by_phandle(node,
						   "samsung,pmu-syscon");
	if (IS_ERR(state->reg_pmu)) {
		dev_err(dev, "failed to lookup PMU syscon\n");
		state->reg_pmu = NULL;
	}
