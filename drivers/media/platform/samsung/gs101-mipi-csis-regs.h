...
/*
 * gs101 MIPI-CSIS v5.4 register map.
 *
 * Register/field tables and the small SFR accessors are taken verbatim from
 * the Samsung "pablo" is-hw-csi-v5_4.h + is-hw-api-common.{h,c} (same CSIS IP
 * as gs101). Kept as a self-contained header so the mainline driver can reuse
 * the exact offsets/bitfields without the pablo type web.
 */
#ifndef GS101_MIPI_CSIS_REGS_H
#define GS101_MIPI_CSIS_REGS_H

#include <linux/io.h>
#include <linux/types.h>

enum regdata_type {
	RW = 0, RO = 1, WO = 2, WI = 2, RAC = 3, W1C = 4, WRI = 5, RWI = 5,
	R_W = 6, RWC = 7, RWS = 8, RIW = 9, ROL = 10, WTC = 11, XWTC = 12,
};
...
static inline u32 is_hw_get_field_value(u32 reg_value, const struct is_field *field)
{
	u32 mask = (field->bit_width >= 32) ? 0xFFFFFFFF :
					      ((1U << field->bit_width) - 1);

	return (reg_value >> field->bit_start) & mask;
}

static inline u32 is_hw_set_field_value(u32 reg_value, const struct is_field *field, u32 val)
{
	u32 mask = (field->bit_width >= 32) ? 0xFFFFFFFF :
					      ((1U << field->bit_width) - 1);

	reg_value &= ~(mask << field->bit_start);
	reg_value |= (val & mask) << field->bit_start;
	return reg_value;
}

static inline u32 is_hw_get_reg(void __iomem *base, const struct is_reg *reg)
{
	return readl(base + reg->sfr_offset);
}

static inline void is_hw_set_reg(void __iomem *base, const struct is_reg *reg, u32 val)
{
	writel(val, base + reg->sfr_offset);
}

static inline u32 is_hw_get_field(void __iomem *base, const struct is_reg *reg,
				  const struct is_field *field)
{
	return is_hw_get_field_value(readl(base + reg->sfr_offset), field);
}

static inline void is_hw_set_field(void __iomem *base, const struct is_reg *reg,
				   const struct is_field *field, u32 val)
{
	u32 v = is_hw_set_field_value(readl(base + reg->sfr_offset), field, val);

	writel(v, base + reg->sfr_offset);
}

/* ---- verbatim register/field tables from pablo is-hw-csi-v5_4.h ---- */
...
/*
 * The PHY register/field tables below are unused by this driver: the CSIS
 * receiver reaches the MIPI D/C-PHY through the generic PHY framework
 * (phy-exynos-mipi-gs101), never these register tables. They are kept
 * verbatim from the vendor is-hw-csi-v5_4.h for reference but compiled out,
 * as they reference CSI_ID_MAX which this mainline port does not define.
 */
#if 0
/* PHY regs */
...
#endif /* 0 -- unused PHY register/field tables */

#endif /* GS101_MIPI_CSIS_REGS_H */
