 ...
 * General device driver for TI LM3644, Dual FLASH LED Driver
 *
 ...
 *
 * Ported to mainline (single-arg probe, OF/fwnode config, gpiod
 * hardware-enable/strobe) for gs101/Pixel 6 camera bring-up.
 */
...
/*  FLASH Brightness: min 11310uA, step 11720uA, max 1500000uA */
...
/*  TORCH Brightness: min 1060uA, step 1460uA, max 187000uA */
...
/*  FLASH TIMEOUT: min 10ms max 400ms; 10ms step to 100ms, 50ms step above */
 ...
 * @lock: mutex for serial access.
 * @enable_gpio: HW enable (HWEN) line, active high; brings chip out of
 *		 hardware shutdown so I2C responds.
 * @strobe_gpio: optional external FLASH strobe line.
 * @ctrls_led: V4L2 controls
 * @subdev_led: V4L2 subdev
 * @led0_enable/led1_enable/flash1_override/torch1_override: chip config,
 *		read from DT/fwnode with sane defaults.
 * @enable_reg: cached enable register (minus mode bits).
 */
struct lm3644_flash {
	struct device *dev;
...
/* drive the optional external strobe line, if present */
static void lm3644_strobe(struct lm3644_flash *flash, bool on)
{
	if (flash->strobe_gpio)
		gpiod_set_value_cansleep(flash->strobe_gpio, on ? 1 : 0);
}
...
static int lm3644_init_device(struct lm3644_flash *flash)
{
	...
	reg_val |= flash->led0_enable;
	reg_val |= flash->led1_enable;
	...
	/* set override */
	rval = regmap_update_bits(flash->regmap,
		  REG_FLASH_LED0_BR, 0x80, flash->flash1_override);
	rval |= regmap_update_bits(flash->regmap,
		  REG_TORCH_LED0_BR, 0x80, flash->torch1_override);
	if (rval < 0)
		return rval;
...
/* read config from DT/fwnode, falling back to chip defaults */
static void lm3644_parse_fwnode(struct lm3644_flash *flash)
{
	struct device *dev = flash->dev;

	/* chip defaults (both LEDs enabled, LED1 current tied to LED0) */
	flash->led0_enable = 0x01;
	flash->led1_enable = 0x01 << 1;
	flash->flash1_override = 0x01 << 7;
	flash->torch1_override = 0x01 << 7;

	if (device_property_read_bool(dev, "ti,led0-disable"))
		flash->led0_enable = 0;
	if (device_property_read_bool(dev, "ti,led1-disable"))
		flash->led1_enable = 0;
	if (device_property_read_bool(dev, "ti,led1-flash-independent"))
		flash->flash1_override = 0;
	if (device_property_read_bool(dev, "ti,led1-torch-independent"))
		flash->torch1_override = 0;
}
...
static int lm3644_probe(struct i2c_client *client)
{
	...
	/*
	 * HW enable (HWEN, active high) must be asserted before the chip
	 * answers on I2C. On gs101 this is a GPIO; the vendor board pulled
	 * it up, mainline drives it explicitly. Optional so I2C-only boards
	 * (HWEN tied high in HW) still probe.
	 */
	flash->enable_gpio = devm_gpiod_get_optional(&client->dev, "enable",
						     GPIOD_OUT_HIGH);
	if (IS_ERR(flash->enable_gpio))
		return dev_err_probe(&client->dev, PTR_ERR(flash->enable_gpio),
				     "failed to get enable gpio\n");
	if (flash->enable_gpio)
		usleep_range(1000, 2000);	/* tHWEN, chip wake */

	/* optional external strobe line (inactive by default) */
	flash->strobe_gpio = devm_gpiod_get_optional(&client->dev, "strobe",
						     GPIOD_OUT_LOW);
	if (IS_ERR(flash->strobe_gpio))
		return dev_err_probe(&client->dev, PTR_ERR(flash->strobe_gpio),
